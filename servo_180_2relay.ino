...
#define RELAY_PIN 6 // PIN to Relay (Program) 
#define RELAY_PINRC 7 // PIN to Relay (RC) 
...
void loop() {  
  ...
  //  Serial.println(relayState);
   ...
   int relayStateRC = digitalRead(RELAY_PINRC); 
   if (relayStateRC == HIGH) {
      Serial.print("Состояние релеRC=");
      Serial.println(relayStateRC);
      angle =  myservo.read();
   //  Serial.print("угол=");
   //  Serial.println(angle);
     if (angle<30) {
      ...
      delay(2000);
      Serial.print("Положение сервопривода из за релеRC=");
      Serial.println(myservo.read());
   }
